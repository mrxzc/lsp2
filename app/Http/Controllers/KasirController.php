...
class KasirController extends Controller
{
    ...
    public function proses_transaksi(Request $r) {
        ...
        if($harga->stok == '1')
        {
            return redirect(route('transaksi'))->with('sukses','saldo anda kurang')
        }
        if($harga->stok == '0'){
    ...
    public function invoice(Request $r){
        ...
        $checkout = Checkout::where('kode_unik', $r->kode_unik)->get();

        return view('kasir.transaksi.bukti', compact('kode_unik','checkout'));
        // $pdf = PDF::loadview('kasir.transaksi.bukti', compact('kode_unik'));
        // return $pdf->stream();
    }
}
